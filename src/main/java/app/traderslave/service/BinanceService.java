...
public class BinanceService {
    ...
    public Mono<ReportOrder> createReportOrder(Simulation simulation, SimulationOrder order, TimeReqDto dto) {
        ...
        if (duration.toHours() <= 180) {
            return createShortTermReport(simulation, order, endTime);
        }

        return createLongTermReport(simulation, order, endTime);
    }

    private Mono<ReportOrder> createShortTermReport(Simulation simulation, SimulationOrder order, LocalDateTime endTime) {
    ...
    private Mono<ReportOrder> createLongTermReport(Simulation simulation, SimulationOrder order, LocalDateTime endTime) {
        ...
        return findCandles(BinanceServiceAdapter.adapt(order.getOpenTime(), simulation, midnightAfterOpening, TimeFrame.ONE_MINUTE))
                .flatMap(candlesRes -> {
                    ...
                    return report.isLiquidated() ? Mono.just(report) : createLongTermReportStepTwo(simulation, order, midnightAfterOpening, endTime, report);
                });
    }

    private Mono<ReportOrder> createLongTermReportStepTwo(Simulation simulation, SimulationOrder order, LocalDateTime midnightAfterOpening, LocalDateTime endTime, ReportOrder rep1) {
        ...
        return findCandles(BinanceServiceAdapter.adapt(midnightAfterOpening, simulation, midnightBeforeEndTime, TimeFrame.ONE_DAY))
                .flatMap(candlesRes -> {
                    ...
                    if (repTimeFrameOneDay.isLiquidated()) {
                        return handleLiquidationCreateLongTermReportStepTwo(simulation, order, endTime, repTimeFrameOneDay);
                    }
                    ReportOrder rep2 = ReportOrderFactory.create(order, candlesRes, repTimeFrameOneDay);
                    return createLongTermReportStepThree(simulation, order, midnightBeforeEndTime, endTime, rep2);
                });
    }

    private Mono<ReportOrder> handleLiquidationCreateLongTermReportStepTwo(Simulation simulation, SimulationOrder order, LocalDateTime endTime, ReportOrder repTimeFrameOneDay) {
    ...
    private Mono<ReportOrder> createLongTermReportStepThree(Simulation simulation, SimulationOrder order, LocalDateTime midnightBeforeEndTime, LocalDateTime endTime, ReportOrder rep1) {
